...
const string strMessageMagic = "Kashmircoin Signed Message:\n";
...
{
    // Kashmircoin: IsDust() detection disabled, allows any valid dust to be relayed.
...

{
	...
	int64_t nSubsidy = 0.001001 * COIN;

	if(nHeight < 1676652) // 

	{

		nSubsidy = 0.0025 * COIN;
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast)
{
    ...
    // Kashmircoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{   
    ...
    int64_t retargetTimespan = nTargetSpacing; //Make sure we retarget every block
    ...
    printf("retargetTimespan = %ld    nActualTimespan = %ld\n", retargetTimespan, nActualTimespan);
    printf("Before: %08x  %s\n", pindexLast->nBits, CBigNum().SetCompact(pindexLast->nBits).getuint256().ToString().c_str());
    printf("After:  %08x  %s\n", bnNew.GetCompact(), bnNew.getuint256().ToString().c_str());
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (fTestNet) {
            ...
            if (pindexLast->nHeight+1 >= 82000) { DiffMode = 2; }
    }
...
bool CBlock::AcceptBlock()
{
	...
	if (IsProofOfWork() && nHeight > LAST3_POW_BLOCK && nHeight < MID3_POW_BLOCK)
        return DoS(100, error("AcceptBlock() : reject proof-of-work at height %d", nHeight));

    if (IsProofOfWork() && nHeight > LAST4_POW_BLOCK)
        return DoS(100, error("AcceptBlock() : reject proof-of-work at height %d", nHeight));
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    if (nSearchTime > nLastCoinStakeSearchTime)
    {
        if (wallet.CreateCoinStake(wallet, nBits, nSearchTime-nLastCoinStakeSearchTime, nFees, txCoinStake, nTxTime, key))
        {
            ...
            {
                // Kashmircoin: since I've had to get rid of CTransaction's nTime,
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Kashmircoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block for Kashmircoin

        const char* pszTimestamp = "NYT: Kerry-Abbas Visit Canceled as Mideast Talks Falter";
        CTransaction txNew;
        ...
        txNew.vout[0].nValue = 0.001001 * COIN;
        ...
        block.nVersion = 1;
        block.nTime    = !fTestNet ? 1396735460 : 1396738142;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = !fTestNet ? 464196 : 532630;

	block.print();
        assert(block.hashMerkleRoot == uint256("0x99f89c41a9ff3c549f8d3bda7221230881cf2fe755422d38284378d625ef1d90"));
